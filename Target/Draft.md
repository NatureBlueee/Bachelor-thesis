本科生毕业论文（设计）

论文标题

AI素养对Z世代员工向上影响行为的影响：资源依赖视角

The Impact of AI Literacy on Upward Influence Behavior among Generation Z Employees: A Resource Dependence Perspective

基本信息

学生姓名：

$$待填写$$

学号：

$$待填写$$

学院（系）：华侨学院

专业（方向）：工商管理

指导教师：

$$待填写$$

完成时间：2026年5月

中文摘要

AI素养对Z世代员工向上影响行为的影响：资源依赖视角

$$学生姓名$$

随着ChatGPT等生成式人工智能工具的快速发展，人工智能素养已成为现代职场中的关键能力。值得注意的是，Z世代员工（1995-2010年出生）作为”数字原住民”，往往比其管理者和资深同事拥有更高水平的AI素养，这种代际技术能力差异有可能重塑组织内部传统的权力动态。本研究基于资源依赖理论，采用定性研究方法，通过对15-20名Z世代知识型员工的半结构化深度访谈，探索AI素养如何影响员工的向上影响行为。研究聚焦于感知资源优势的中介作用以及上级开放性的调节作用，旨在揭示AI时代年轻员工获得组织影响力的微观机制。研究发现将为AI驱动型组织的人才管理和代际协作提供理论洞见与实践建议。

关键词： AI素养；向上影响行为；资源依赖理论；Z世代员工；感知资源优势

ABSTRACT

The Impact of AI Literacy on Upward Influence Behavior among Generation Z Employees: A Resource Dependence Perspective

$$Student Name$$

With the rapid advancement of generative artificial intelligence technologies such as ChatGPT, AI literacy has become a critical competency in the modern workplace. Notably, Generation Z employees (born 1995-2010), as “digital natives,” often possess higher levels of AI literacy compared to their managers and senior colleagues, creating a generational asymmetry that has the potential to reshape traditional power dynamics within organizations. Grounded in Resource Dependence Theory, this study adopts a qualitative research approach, utilizing semi-structured in-depth interviews with 15-20 Generation Z knowledge workers to explore how AI literacy influences upward influence behavior. The research focuses on the mediating role of perceived resource advantage and the moderating role of supervisor openness, aiming to reveal the micro-mechanisms through which young employees acquire organizational influence in the AI era. The findings will provide theoretical insights and practical recommendations for talent management and intergenerational collaboration in AI-driven organizations.

Key words: AI Literacy; Upward Influence Behavior; Resource Dependence Theory; Generation Z Employees; Perceived Resource Advantage

Table of Contents

CHAPTER I INTRODUCTION ………………………………………………… 1

1.1 Research Background ………………………………………………… 1

1.2 Research Objectives ………………………………………………… 3

1.3 Research Significance ………………………………………………… 4

CHAPTER II LITERATURE REVIEW ………………………………………………… 5

2.1 Resource Dependence Theory and Upward Influence ………………………………………………… 5

2.2 AI Literacy: Conceptualization and Frameworks ………………………………………………… 7

2.3 Upward Influence Behavior in Organizations ………………………………………………… 9

2.4 Generation Z in the Workplace ………………………………………………… 11

2.5 Research Gaps and Theoretical Framework ………………………………………………… 12

CHAPTER III RESEARCH METHODOLOGY ………………………………………………… 14

3.1 Research Design ………………………………………………… 14

3.2 Sample and Sampling Strategy ………………………………………………… 14

3.3 Data Collection ………………………………………………… 15

3.4 Interview Guide ………………………………………………… 16

3.5 Data Analysis ………………………………………………… 17

3.6 Quality Assurance ………………………………………………… 18

CHAPTER IV FINDINGS ………………………………………………… 19

4.1 

$$待完成：访谈发现$$

4.2 

$$待完成：主题分析结果$$

CHAPTER V DISCUSSION ………………………………………………… 20

5.1 

$$待完成：研究发现讨论$$

5.2 

$$待完成：理论贡献$$

CHAPTER VI CONCLUSION AND IMPLICATIONS ………………………………………………… 21

6.1 

$$待完成：研究结论$$

6.2 

$$待完成：实践启示$$

6.3 

$$待完成：研究局限与未来方向$$

REFERENCES ………………………………………………… 22

APPENDIXES ………………………………………………… 25

Appendix I: Interview Guide

Appendix II: Informed Consent Form

ACKNOWLEDGEMENTS ………………………………………………… 28

List of Tables

Table 2.1 Comparison of AI Literacy Frameworks ………………………………………………… 8

Table 2.2 Dimensions of Upward Influence Behavior ………………………………………………… 10

Table 2.3 Summary of Research Propositions ………………………………………………… 13

Table 3.1 Participant Selection Criteria ………………………………………………… 15

Table 3.2 Interview Guide Structure ………………………………………………… 16

Table 4.1 

$$待完成：受访者基本信息$$

 ………………………………………………… 19

Table 4.2 

$$待完成：主题编码结果$$

 ………………………………………………… 19

List of Figures

Figure 2.1 Research Framework ………………………………………………… 13

Figure 3.1 Data Analysis Process ………………………………………………… 18

CHAPTER I INTRODUCTION

1.1 Research Background

1.1.1 The Rise of AI in the Workplace

The rapid advancement of artificial intelligence (AI) technologies, exemplified by the emergence of ChatGPT, Claude, and other generative AI tools, has fundamentally transformed the modern workplace. Since the public release of ChatGPT in November 2022, AI-powered tools have been rapidly adopted across industries, enabling employees to automate routine tasks, enhance creative output, and improve decision-making processes. In China, the national “AI+” strategy has further accelerated the integration of AI into organizational processes, positioning AI literacy as an increasingly critical competency for employees in the digital economy.

1.1.2 Generational Asymmetry in AI Competence

A distinctive feature of the current AI revolution is the generational asymmetry in technological competence. Generation Z employees, typically defined as those born between 1995 and 2010, have grown up immersed in digital technologies and are often characterized as “digital natives” (Prensky, 2001). Research suggests that younger employees tend to adopt new technologies more quickly and exhibit higher levels of digital fluency compared to their older counterparts (Ng et al., 2023). This creates an interesting power dynamic in organizations: while managers and senior employees typically hold formal authority and institutional knowledge, younger employees may possess superior AI skills that are increasingly valuable in the workplace.

1.1.3 Shifting Power Dynamics

This generational asymmetry in AI competence has the potential to reshape traditional power dynamics within organizations. According to Resource Dependence Theory (Pfeffer & Salancik, 1978), power in organizations derives from the control of scarce and valuable resources. When Generation Z employees possess AI skills that their supervisors lack, they may acquire a form of resource-based power that enables them to exert upward influence on their managers. This phenomenon challenges conventional top-down leadership models and raises important questions about how AI literacy translates into organizational influence.

1.1.4 The Chinese Context

The Chinese organizational context presents unique characteristics that make this research particularly relevant. China’s high power distance culture (Hofstede, 2001) traditionally emphasizes hierarchical relationships and deference to authority. However, the rapid pace of digital transformation in Chinese organizations, combined with the large proportion of Generation Z entering the workforce, creates tensions between traditional power structures and emerging technology-driven influence patterns. Understanding how AI literacy enables upward influence in this context can provide valuable insights for both theory and practice.

1.2 Research Objectives

This study aims to achieve the following objectives:

(1) Explore how AI literacy enables Generation Z employees to engage in upward influence behavior in the workplace, identifying the specific mechanisms and processes involved.

(2) Investigate the role of perceived resource advantage as a psychological mechanism linking AI literacy to upward influence behavior, understanding how employees’ subjective perceptions mediate this relationship.

(3) Examine how supervisor openness moderates the relationship between AI literacy and upward influence behavior, considering the boundary conditions of this phenomenon.

(4) Provide theoretical insights grounded in Resource Dependence Theory and practical recommendations for talent management and intergenerational collaboration in AI-driven organizations.

1.3 Research Significance

1.3.1 Theoretical Significance

This study makes several theoretical contributions to the organizational behavior literature. First, it extends Resource Dependence Theory (Pfeffer & Salancik, 1978) to the employee-supervisor dyad by examining how AI literacy, as a form of firm-specific human capital, creates dependency relationships that enable upward influence. While RDT has traditionally been applied at the organizational level to explain inter-organizational power dynamics, recent work by Tripathi (2021) has demonstrated its applicability to within-organization relationships. This study builds on this foundation by examining AI literacy as a specific resource that shifts power dynamics.

Second, this study contributes to the emerging literature on AI literacy by exploring its organizational behavior outcomes beyond educational settings. While existing research on AI literacy has primarily focused on its conceptualization and measurement (Ng et al., 2023; Zhong et al., 2024), less attention has been paid to how AI literacy influences workplace dynamics such as power, influence, and leadership relationships.

Third, this study advances the literature on upward influence by identifying AI literacy as a novel antecedent of influence behavior, particularly in the context of generational dynamics and digital transformation.

1.3.2 Practical Significance

The findings of this study will have important practical implications for organizations navigating digital transformation. For human resource managers, understanding how AI literacy translates into organizational influence can inform talent development strategies and succession planning. For organizational leaders, the findings can guide the creation of organizational cultures that leverage the AI competencies of younger employees while maintaining effective intergenerational collaboration.

In the Chinese context, where organizations face the dual challenges of rapid technological change and evolving workforce demographics, this research can provide actionable insights for managing the transition to AI-driven work environments while respecting cultural norms around hierarchy and authority.

CHAPTER II LITERATURE REVIEW

2.1 Resource Dependence Theory and Upward Influence

2.1.1 Theoretical Foundations of Resource Dependence Theory

Resource Dependence Theory (RDT), originally developed by Pfeffer and Salancik (1978) in their seminal work “The External Control of Organizations,” provides a powerful framework for understanding power dynamics in organizational contexts. At its core, RDT posits that power is fundamentally about the control of resources that others need but cannot easily obtain elsewhere. As Pfeffer and Salancik articulated, when Actor A possesses resources that Actor B needs but lacks, B becomes dependent on A, and A acquires a power base for influencing B.

The theory identifies several key factors that determine the extent of resource-based power: (1) the importance of the resource to the dependent party, (2) the scarcity of the resource, (3) the concentration of resource control, and (4) the availability of alternative sources. When a resource is both important and scarce, and when alternatives are limited, the party controlling that resource gains significant influence over those who depend on it.

2.1.2 Extending RDT to Employee-Supervisor Relationships

While RDT was initially developed to explain inter-organizational relationships, recent scholarship has extended its application to within-organization dynamics. Tripathi (2021), in a groundbreaking theoretical paper published in Frontiers in Psychology, argued that the same principles of resource dependence can explain the power dynamics between employees and their supervisors. Tripathi introduced the concept of employees as “hosts of tacit resources”—individuals who possess unique, firm-specific human capital that managers need but cannot easily replicate.

According to Tripathi’s framework, when employees hold valuable and scarce resources, they can engage in “claiming” behaviors—actively asserting their competence and seeking recognition from supervisors. This aligns with the Leadership Construction Theory proposed by DeRue and Ashford (2010), which suggests that leadership is not solely a top-down phenomenon but is co-constructed through the interaction between “claiming” (asserting leadership identity) and “granting” (receiving recognition from others).

2.1.3 Tripathi’s Framework: Employees as Resource Holders

Tripathi’s (2021) theoretical framework, published in Frontiers in Psychology, provides the most direct application of Resource Dependence Theory to the employee-supervisor relationship, and serves as the primary theoretical foundation for this study. The framework introduces several key concepts that are central to understanding how AI literacy enables upward influence.

First, Tripathi conceptualizes employees as “hosts of tacit resources”—individuals who possess unique, firm-specific human capital that is valuable to managers but difficult to replicate or transfer. Unlike generic skills that can be easily hired from the external labor market, firm-specific human capital is developed through experience within a particular organizational context and becomes increasingly valuable over time. In the context of AI, this tacit knowledge includes not only technical AI skills but also understanding of how to apply AI tools to specific organizational problems and workflows.

Second, the framework distinguishes between two types of dependency relationships: power imbalance and embedded (or joint) dependence. Power imbalance occurs when one party is significantly more dependent on the other, creating an asymmetric relationship. Embedded dependence, in contrast, involves mutual reliance where both parties need each other. The AI literacy context often creates power imbalance in favor of younger employees who possess scarce technological competencies that their supervisors lack.

Third, Tripathi emphasizes that the conversion of resources into influence depends on several boundary conditions, including organizational structure (mechanistic vs. organic) and team composition (expert vs. homogeneous). This insight informs the inclusion of supervisor openness as a moderating variable in this study—the extent to which supervisors are receptive to employee input represents a critical contextual factor that shapes whether resource advantages translate into actual influence.

Finally, the framework connects to Leadership Construction Theory (DeRue & Ashford, 2010) by highlighting the interactive nature of influence. Employees must engage in “claiming” behaviors—actively asserting their competence and seeking opportunities to contribute—while supervisors must engage in “granting” behaviors—acknowledging employee expertise and providing opportunities for input. This dual process suggests that AI literacy alone is insufficient; employees must also perceive their advantage and have supervisors who are open to their contributions.

2.2 AI Literacy: Conceptualization and Frameworks

2.2.1 Defining AI Literacy

AI literacy has emerged as a critical concept in both educational and organizational research. Unlike general digital literacy, which encompasses broad competencies related to information technology, AI literacy specifically addresses the knowledge, skills, attitudes, and ethical awareness needed to effectively interact with AI systems (Long & Magerko, 2020).

Several frameworks have been proposed to conceptualize AI literacy. Ng et al. (2023) developed the ABCE model, which includes four dimensions: Affective (attitudes toward AI), Behavioral (ability to use AI tools), Cognitive (understanding of AI concepts), and Ethical (awareness of AI-related ethical issues). This framework was validated with Hong Kong secondary students and has been applied in various educational contexts.

2.2.2 Zhong’s Three-Dimensional Framework

Particularly relevant to the Chinese context is the three-dimensional framework proposed by Zhong et al. (2024) in their article “What is AI Literacy: Essence, Composition, and Evaluation System” published in the Journal of East China Normal University. Zhong’s framework conceptualizes AI literacy as comprising three interrelated dimensions:

(1) AI Knowledge (AI知识): This dimension encompasses declarative and procedural knowledge about AI, including understanding of AI concepts, technologies, applications, and limitations. It represents the cognitive foundation of AI literacy.

(2) AI Affectivity (AI情感): This dimension captures emotional and attitudinal responses to AI, including interest, confidence, and value perceptions. Affective engagement with AI influences motivation to learn and use AI tools.

(3) AI Thinking (AI思维): This dimension reflects higher-order cognitive skills related to AI, including computational thinking, engineering design thinking, and critical evaluation of AI systems. It represents the ability to think with and about AI.

Zhong’s framework is grounded in Piaget’s genetic epistemology and Bloom’s taxonomy of educational objectives, providing a theoretically robust foundation. The framework emphasizes the dynamic transformation among the three dimensions, suggesting that AI literacy develops through the interplay of knowing, feeling, and thinking about AI.

2.2.3 AI Literacy in Organizational Contexts

While most research on AI literacy has focused on educational settings, Cetindamar et al. (2022) made an important contribution by explicating AI literacy in the context of digital workplaces. Their study, published in IEEE Transactions on Engineering Management, identified key competencies that employees need to effectively use AI tools in professional settings. Wang et al. (2022) further developed and validated an AI literacy scale specifically designed for general users, demonstrating good reliability (Cronbach’s α = 0.906) in Chinese samples.

Table 2.1 Comparison of AI Literacy Frameworks

|

| Framework | Dimensions | Context | Key Features |
| Ng et al. (2023) ABCE Model | Affective, Behavioral, Cognitive, Ethical | Education (Hong Kong) | 32 items, validated with secondary students |
| Zhong et al. (2024) Three-Dimensional | AI Knowledge, AI Affectivity, AI Thinking | Education (China) | 48 items, grounded in Piaget and Bloom |
| Wang et al. (2022) | Cognitive, Use, Evaluation, Ethics | General users (China) | 12 items, α = 0.906 |
| Cetindamar et al. (2022) | Multiple competencies | Workplace | Exploratory, qualitative approach |

2.3 Upward Influence Behavior in Organizations

2.3.1 Conceptualizing Upward Influence

Upward influence refers to employees’ attempts to affect the attitudes, perceptions, or behaviors of their supervisors or other organizational members who hold higher positions (Kipnis et al., 1980). Unlike downward influence, which flows from managers to subordinates and is often supported by formal authority, upward influence requires employees to rely on alternative power bases such as expertise, information control, or personal relationships.

The study of upward influence has a rich tradition in organizational behavior research. Kipnis, Schmidt, and Wilkinson (1980) developed the foundational taxonomy of influence tactics, identifying strategies such as rational persuasion, ingratiation, coalition building, and pressure. Subsequent research has refined this taxonomy and examined the antecedents, processes, and outcomes of upward influence across different contexts.

2.3.2 Dimensions of Upward Influence Behavior

Building on Scott and Bruce’s (1994) conceptualization of innovation behavior, upward influence can be understood as encompassing multiple stages or dimensions:

(1) Idea Generation: The initial stage involves identifying opportunities for improvement or innovation and developing ideas that could benefit the organization. In the context of AI literacy, this might involve recognizing how AI tools could enhance work processes.

(2) Idea Promotion: Once ideas are generated, employees must advocate for them to gain support from supervisors and other stakeholders. This requires communication skills, political awareness, and the ability to frame ideas in ways that resonate with organizational priorities.

(3) Idea Implementation: The final stage involves translating approved ideas into action. This may require coordinating with others, overcoming resistance, and demonstrating the value of the proposed changes.

Table 2.2 Dimensions of Upward Influence Behavior

| Dimension | Definition | AI Literacy Application |
| Idea Generation | Identifying opportunities and developing solutions | Recognizing how AI can improve work processes |
| Idea Promotion | Advocating for ideas to gain support | Communicating AI benefits to supervisors |
| Idea Implementation | Translating ideas into action | Leading AI adoption initiatives |

2.3.3 Antecedents of Upward Influence

Research has identified various factors that enable or constrain upward influence behavior. At the individual level, factors such as self-efficacy (Bandura, 1997), political skill, and expertise have been shown to predict influence attempts. At the relational level, the quality of leader-member exchange (LMX) and trust between employees and supervisors affect the likelihood and success of upward influence.

Particularly relevant to this study is the role of expertise as a power base for upward influence. When employees possess specialized knowledge or skills that supervisors lack, they gain “expert power” that can be leveraged to influence organizational decisions (French & Raven, 1959). AI literacy, as a form of technological expertise that is often more developed among younger employees, represents a contemporary manifestation of this dynamic.

2.4 Generation Z in the Workplace

2.4.1 Characteristics of Generation Z

Generation Z, typically defined as individuals born between 1995 and 2010, represents the newest cohort entering the workforce. As true “digital natives,” Gen Z employees have grown up with smartphones, social media, and ubiquitous internet connectivity. This has shaped their cognitive styles, communication preferences, and expectations for work (Seemiller & Grace, 2016).

Research on Gen Z in the workplace has identified several distinctive characteristics. Gen Z employees tend to be technologically fluent, comfortable with multitasking, and expect to use digital tools in their work. They value authenticity, diversity, and meaningful work, and may be more willing than previous generations to challenge organizational norms that they perceive as outdated or inefficient (Schroth, 2019).

2.4.2 Generational Dynamics and Power

The entry of Generation Z into the workforce has created interesting dynamics around knowledge and power. In traditional organizations, knowledge and expertise typically accumulated with tenure, reinforcing the authority of senior employees. However, the rapid pace of technological change has disrupted this pattern. In the domain of AI and digital technologies, younger employees often possess superior skills, creating what some scholars have termed a “reverse knowledge gap.”

This dynamic has implications for power relationships in organizations. According to Resource Dependence Theory, when younger employees control resources (i.e., AI competencies) that are valuable to supervisors and the organization, they acquire influence that would not traditionally be available to new or junior employees. This represents a potential shift in organizational power dynamics that merits empirical investigation.

2.4.3 The Chinese Organizational Context

The Chinese organizational context presents unique characteristics that make this research particularly relevant and theoretically interesting. China is characterized by high power distance (Hofstede, 2001), meaning that hierarchical relationships and deference to authority are culturally emphasized. In traditional Chinese organizations, subordinates are expected to show respect for their superiors and may be hesitant to challenge or openly influence those in positions of authority.

However, several contemporary forces are creating tensions with these traditional norms. First, China’s rapid digital transformation, accelerated by the national “AI+” strategy, has created urgent demand for AI competencies that younger employees are more likely to possess. Second, the large cohort of Generation Z entering the workforce brings different expectations about workplace dynamics, including greater comfort with challenging established practices. Third, multinational companies and technology firms in China often adopt flatter organizational structures that may encourage more open communication across hierarchical levels.

These tensions create a particularly interesting context for studying AI-enabled upward influence. On one hand, cultural norms may suppress influence attempts even when employees possess valuable resources. On the other hand, the strategic importance of AI competencies may override traditional hierarchical constraints, creating new pathways for young employees to gain voice and influence. The moderating role of supervisor openness becomes especially critical in this context, as it represents a potential bridge between traditional hierarchical expectations and emerging technology-driven influence patterns.

2.5 Research Gaps and Theoretical Framework

2.5.1 Identified Research Gaps

Despite growing interest in AI literacy and generational dynamics in the workplace, several important research gaps remain:

First, while AI literacy has been extensively studied in educational contexts, its implications for organizational behavior—particularly power and influence dynamics—remain underexplored. How AI literacy translates into workplace influence represents an important question for both theory and practice.

Second, although Resource Dependence Theory provides a compelling framework for understanding power dynamics, few studies have specifically examined AI literacy as a resource that enables upward influence. The mechanisms by which AI competencies are converted into organizational influence warrant investigation.

Third, the role of contextual factors, particularly supervisor attitudes toward employee input and new technologies, in shaping the AI literacy-influence relationship has received limited attention. Understanding these boundary conditions is crucial for developing a nuanced theory of AI-enabled upward influence.

2.5.2 Theoretical Framework and Research Propositions

Based on the literature review, this study proposes a theoretical framework linking AI literacy to upward influence behavior through the mediating mechanism of perceived resource advantage, with supervisor openness as a moderating factor.

$$Insert Figure 2.1 Research Framework Here$$

The framework is grounded in Resource Dependence Theory and proposes the following relationships:

Proposition 1 (P1): AI literacy positively relates to employees’ perceived resource advantage.

When employees possess high levels of AI literacy—including AI knowledge, AI affectivity, and AI thinking—they are more likely to perceive themselves as holding a resource advantage relative to their supervisors. This perception is particularly strong when employees recognize that their AI competencies exceed those of their managers and are valued by the organization.

Proposition 2 (P2): Perceived resource advantage enables employees to engage in upward influence behavior.

Building on Social Cognitive Theory (Bandura, 1997), the perception of holding valuable resources should increase employees’ self-efficacy and motivation to attempt upward influence. When employees believe they possess something that supervisors need, they are more likely to engage in idea generation, promotion, and implementation behaviors.

Proposition 3 (P3): Perceived resource advantage mediates the relationship between AI literacy and upward influence behavior.

The effect of AI literacy on upward influence is not direct but operates through the psychological mechanism of perceived resource advantage. Objective AI competencies must be subjectively recognized as conferring an advantage before they translate into influence behavior.

Proposition 4 (P4): Supervisor openness moderates the relationship between perceived resource advantage and upward influence behavior.

Drawing on Leadership Construction Theory (DeRue & Ashford, 2010), the success of upward influence depends not only on the employee’s claiming behavior but also on the supervisor’s granting behavior. In the Chinese high power distance context, supervisor openness—defined as the extent to which supervisors are receptive to employee input and new ideas (Detert & Burris, 2007)—should significantly moderate whether perceived resource advantage translates into actual influence attempts and success.

2.5.3 Summary of Research Propositions

Based on the theoretical framework developed above, this study proposes the following research propositions to guide the qualitative inquiry:

Proposition 1 (P1): AI Literacy → Perceived Resource Advantage

Generation Z employees with higher levels of AI literacy (encompassing AI knowledge, AI affectivity, and AI thinking) are more likely to perceive themselves as holding a resource advantage relative to their supervisors. This proposition is grounded in Resource Dependence Theory’s assertion that controlling scarce, valuable resources creates a power base.

Proposition 2 (P2): Perceived Resource Advantage → Upward Influence Behavior

Employees who perceive that they possess AI-related resources that their supervisors need but lack are more likely to engage in upward influence behavior, including generating new ideas, promoting ideas to leadership, and implementing changes. This proposition draws on Social Cognitive Theory (Bandura, 1997), which suggests that self-efficacy beliefs—in this case, confidence derived from perceived resource advantage—drive behavioral action.

Proposition 3 (P3): Mediation Effect

The relationship between AI literacy and upward influence behavior is mediated by perceived resource advantage. That is, objective AI competencies must first be subjectively recognized as conferring an advantage before they motivate influence attempts. This proposition reflects the psychological mechanism through which external resources are translated into behavioral outcomes.

Proposition 4 (P4): Moderation Effect of Supervisor Openness

The relationship between perceived resource advantage and upward influence behavior is moderated by supervisor openness. When supervisors are receptive to input from subordinates and open to new technologies, employees with perceived resource advantages are more likely to engage in upward influence. Conversely, when supervisors are closed to input, even employees with significant resource advantages may refrain from influence attempts. This proposition acknowledges the relational nature of influence and the importance of contextual factors, particularly in high power distance cultures like China.

Table 2.3 Summary of Research Propositions

| Proposition | Relationship | Theoretical Basis |
| P1 | AI Literacy → Perceived Resource Advantage | Resource Dependence Theory |
| P2 | Perceived Resource Advantage → Upward Influence Behavior | Social Cognitive Theory |
| P3 | AI Literacy → Perceived Resource Advantage → Upward Influence Behavior (Mediation) | RDT + SCT Integration |
| P4 | Supervisor Openness moderates P2 | Leadership Construction Theory |

CHAPTER III RESEARCH METHODOLOGY

3.1 Research Design and Justification

This study adopts a qualitative research approach, specifically utilizing semi-structured in-depth interviews, to explore how AI literacy influences upward influence behavior among Generation Z employees. The choice of qualitative methodology over quantitative approaches is justified by several methodological and practical considerations.

Exploratory Nature of the Research Question

The phenomenon under investigation—how AI literacy translates into organizational influence through perceived resource advantage—is relatively novel and has not been extensively studied empirically. While Resource Dependence Theory provides a general framework for understanding power dynamics, its specific application to AI literacy and generational dynamics in the workplace represents new theoretical territory. Qualitative methods are well-suited for such exploratory research, allowing for the discovery of unexpected themes and the development of grounded theoretical insights (Creswell & Poth, 2018).

Need for Process Understanding

This study seeks to understand not merely whether AI literacy relates to upward influence, but how this relationship operates—the mechanisms, processes, and contextual factors involved. Quantitative methods, while valuable for testing causal relationships, may not adequately capture the richness of employees’ experiences, perceptions, and interpretations. In-depth interviews allow participants to describe their experiences in their own words, revealing nuances that predetermined survey items might miss.

Alignment with Institutional Emphasis

This research approach aligns with the emphasis on field research and social investigation in undergraduate education at this institution. Conducting qualitative interviews provides valuable opportunities for direct engagement with research participants and the development of practical research skills.

Methodological Precedent

Qualitative methods have been successfully employed in prior research on related topics, including studies of generational dynamics in the workplace (Watkins & Omilion-Hodges, 2025), upward influence processes (Kamaşak et al., 2017), and AI adoption in organizations (Cassell & Symon, 2004). These precedents support the appropriateness of qualitative inquiry for the current research questions.

3.2 Sample and Sampling Strategy

3.2.1 Target Population

The target population for this study consists of Generation Z knowledge workers in China who use AI tools in their daily work. Knowledge workers are defined as employees whose primary contribution involves the creation, distribution, or application of knowledge, as opposed to manual or routine clerical work.

3.2.2 Sampling Criteria

Participants will be selected based on the following criteria:

Table 3.1 Participant Selection Criteria

| Criterion | Requirement | Rationale |
| Age/Generation | Born 1995-2010 (Generation Z) | Focus on digital natives |
| Employment | At least 6 months full-time work experience | Ensure sufficient workplace exposure |
| AI Usage | Regular use of AI tools at work | Ensure relevance to research topic |
| Job Type | Knowledge work (non-manual) | Focus on contexts where AI literacy is relevant |

3.2.3 Sample Size and Saturation

Following the principle of theoretical saturation in qualitative research (Guest et al., 2020), this study targets a sample size of 15-20 participants. Research indicates that for relatively homogeneous samples, thematic saturation is typically achieved within 12-20 interviews, with 80% or more of themes emerging within the first 6-12 interviews (Guest et al., 2020; Malterud et al., 2016; Hennink & Kaiser, 2021).

The sample size will be confirmed through ongoing assessment of saturation during data collection. If new themes continue to emerge after 15 interviews, additional participants will be recruited until saturation is achieved.

3.2.4 Sampling Strategy

This study employs purposive sampling combined with snowball sampling. Initial participants will be identified through the researcher’s professional and academic networks, targeting individuals who meet the selection criteria. Subsequently, participants will be asked to recommend other potential participants who might be interested in the study (snowball sampling).

To enhance sample diversity, the researcher will seek participants from different industries, company sizes, and functional areas. This variation will enable exploration of how contextual factors shape the AI literacy-upward influence relationship.

3.3 Data Collection

3.3.1 Interview Method

Data will be collected through semi-structured in-depth interviews. This method allows for systematic coverage of key topics while providing flexibility to explore unexpected themes and follow participants’ narratives (Kvale & Brinkmann, 2009).

Each interview is expected to last 45-60 minutes and will be conducted either in person or via video call, depending on participant preference and logistical considerations. Interviews will be audio-recorded with participant consent and subsequently transcribed verbatim for analysis.

3.3.2 Interview Procedure

The interview procedure will follow these steps:

(1) Pre-interview: Send informed consent form and brief overview of topics to participants.

(2) Opening (5 minutes): Establish rapport, explain study purpose, confirm consent, address questions.

(3) Main interview (40-50 minutes): Work through interview guide, probing for depth and examples.

(4) Closing (5 minutes): Summarize key points, invite additional comments, explain next steps.

(5) Post-interview: Record researcher reflections and observations in field notes.

3.4 Interview Guide

The interview guide is structured around the key constructs in the research framework, moving from general to specific topics.

Table 3.2 Interview Guide Structure

| Topic Area | Sample Questions | Construct |
| Background | “Please describe your current role and how you use AI tools in your work.” | Context |
| AI Literacy | “How did you learn to use AI tools? How would you compare your AI skills to your colleagues and supervisor?” | AI Literacy |
| Perceived Advantage | “Do you feel you have any advantages over your supervisor when it comes to AI? Can you give examples?” | Perceived Resource Advantage |
| Upward Influence | “Have you ever used your AI knowledge to influence your supervisor’s decisions or the way work is done? What happened?” | Upward Influence Behavior |
| Supervisor Response | “How does your supervisor typically respond when you make AI-related suggestions? What factors affect their openness?” | Supervisor Openness |
| Contextual Factors | “What aspects of your organization’s culture help or hinder you in making suggestions to leadership?” | Boundary Conditions |

Detailed Interview Questions

Opening Questions (5 minutes)

Please briefly introduce yourself and your current work responsibilities.

How would you describe your experience with AI tools in your daily work?

AI Literacy Exploration (10 minutes)

How did you first learn to use AI tools like ChatGPT or others?

What AI tools do you use most frequently at work? For what purposes?

How would you rate your AI skills compared to your colleagues? Compared to your supervisor?

What is your general attitude toward AI technology? Any concerns or expectations?

Perceived Resource Advantage (10 minutes)

Do you feel that you have capabilities in AI that your supervisor lacks?

How did this perception develop? Can you describe specific situations?

How does this awareness affect how you see yourself in the organization?

Upward Influence Behavior (15 minutes)

Have there been times when your AI knowledge helped you influence your supervisor’s decisions or behaviors?

Can you describe a specific situation in detail? What did you do? What was the outcome?

What factors encouraged or discouraged you from attempting to influence your supervisor?

How do you typically approach making suggestions to your supervisor about AI-related matters?

Supervisor Openness (5 minutes)

How does your supervisor typically react to your AI-related suggestions?

Would you describe your supervisor as open to learning from younger employees? Why or why not?

How does your supervisor’s attitude affect your willingness to speak up?

Closing (5 minutes)

Is there anything else you would like to add about your experiences with AI at work?

Looking forward, how do you see your AI skills affecting your career development?

3.5 Data Analysis

3.5.1 Analytical Approach

Data will be analyzed using thematic analysis, following the six-phase approach outlined by Braun and Clarke (2006) and further developed by Naeem et al. (2023):

Phase 1: Familiarization with Data

Read and re-read all interview transcripts

Note initial ideas and observations

Become immersed in the data

Phase 2: Generating Initial Codes

Systematically code interesting features across the entire dataset

Collate data relevant to each code

Use both inductive (data-driven) and deductive (theory-driven) coding

Phase 3: Searching for Themes

Collate codes into potential themes

Gather all data relevant to each potential theme

Create initial thematic maps

Phase 4: Reviewing Themes

Check if themes work in relation to coded extracts and entire dataset

Generate a thematic map of the analysis

Refine themes as needed

Phase 5: Defining and Naming Themes

Refine the specifics of each theme

Generate clear definitions and names for each theme

Identify the “story” each theme tells

Phase 6: Producing the Report

Select vivid, compelling extract examples

Relate analysis back to research questions and literature

Produce a scholarly report of the analysis

3.5.2 Coding Strategy

The coding process will combine deductive and inductive approaches. Deductive codes will be derived from the theoretical framework, including categories for AI literacy dimensions, perceived resource advantage, upward influence behaviors, and supervisor openness. Inductive codes will emerge from the data to capture unexpected themes and nuances.

A codebook will be developed and refined throughout the analysis process to ensure consistency and transparency (Roberts et al., 2019).

3.6 Quality Assurance

To ensure the rigor and trustworthiness of this qualitative study, the following criteria and strategies will be employed:

3.6.1 Credibility

Credibility refers to the accuracy of the findings from the standpoint of the researcher, participants, and readers. Strategies to enhance credibility include:

Member checking: Sharing preliminary findings with select participants to verify accuracy

Peer debriefing: Discussing findings with academic advisors and peers

Prolonged engagement: Spending sufficient time in data collection and analysis

3.6.2 Transferability

Transferability refers to the extent to which findings can be applied to other contexts. This will be addressed through:

Thick description: Providing detailed descriptions of the research context, participants, and processes

Clear sampling criteria: Enabling readers to assess the applicability to other settings

3.6.3 Dependability

Dependability refers to the consistency and reliability of the research process. Strategies include:

Audit trail: Maintaining detailed records of all research decisions and processes

Research journal: Keeping a reflexive journal throughout the study

3.6.4 Confirmability

Confirmability refers to the neutrality and accuracy of the findings. This will be ensured through:

Researcher reflexivity: Acknowledging and reflecting on the researcher’s own biases and assumptions

Data triangulation: Comparing findings across different participants and contexts

Quotation use: Supporting interpretations with direct quotes from participants

CHAPTER IV FINDINGS

$$待完成：数据收集和分析后填写$$

4.1 Participant Overview

$$待完成：受访者基本信息表格$$

4.2 Thematic Analysis Results

$$待完成：主题分析结果$$

4.2.1 Theme 1: 

$$待确定$$

$$待完成$$

4.2.2 Theme 2: 

$$待确定$$

$$待完成$$

4.2.3 Theme 3: 

$$待确定$$

$$待完成$$

CHAPTER V DISCUSSION

$$待完成：数据分析后填写$$

5.1 Summary of Key Findings

$$待完成$$

5.2 Theoretical Contributions

$$待完成$$

5.3 Comparison with Prior Research

$$待完成$$

CHAPTER VI CONCLUSION AND IMPLICATIONS

$$待完成$$

6.1 Research Conclusions

$$待完成$$

6.2 Practical Implications

$$待完成$$

6.3 Limitations and Future Research Directions

$$待完成$$

REFERENCES

Bandura, A. (1997). Self-efficacy: The exercise of control. W.H. Freeman.

Braun, V., & Clarke, V. (2006). Using thematic analysis in psychology. Qualitative Research in Psychology, 3(2), 77-101. https://doi.org/10.1191/1478088706qp063oa

Cassell, C., & Symon, G. (2004). Essential guide to qualitative methods in organizational research. SAGE Publications. https://doi.org/10.4135/9781446280119

Cetindamar, D., Kitto, K., Wu, M., Zhang, Y., Abedin, B., & Knight, S. (2022). Explicating AI literacy of employees at digital workplaces. IEEE Transactions on Engineering Management, 69(6), 3144-3158. https://doi.org/10.1109/TEM.2021.3138503

Creswell, J. W., & Poth, C. N. (2018). Qualitative inquiry and research design: Choosing among five approaches (4th ed.). SAGE Publications.

DeRue, D. S., & Ashford, S. J. (2010). Who will lead and who will follow? A social process of leadership identity construction in organizations. Academy of Management Review, 35(4), 627-647. https://doi.org/10.5465/amr.35.4.zok627

Detert, J. R., & Burris, E. R. (2007). Leadership behavior and employee voice: Is the door really open? Academy of Management Journal, 50(4), 869-884. https://doi.org/10.5465/amj.2007.26279183

French, J. R. P., & Raven, B. (1959). The bases of social power. In D. Cartwright (Ed.), Studies in social power (pp. 150-167). Institute for Social Research.

Guest, G., Namey, E., & Chen, M. (2020). A simple method to assess and report thematic saturation in qualitative research. PLoS ONE, 15(5), e0232076. https://doi.org/10.1371/journal.pone.0232076

Hennink, M., & Kaiser, B. (2021). Sample sizes for saturation in qualitative research: A systematic review of empirical tests. Social Science & Medicine, 292, 114523. https://doi.org/10.1016/j.socscimed.2021.114523

Hofstede, G. (2001). Culture’s consequences: Comparing values, behaviors, institutions, and organizations across nations (2nd ed.). SAGE Publications.

Kamaşak, R., Kar, A., Yavuz, M., & Baykut, S. (2017). Qualitative methods in organizational research: An example of grounded theory data analysis. In Handbook of research on organizational culture and diversity in the modern workforce (pp. 23-42). IGI Global. https://doi.org/10.4018/978-1-5225-2250-8.ch002

Kipnis, D., Schmidt, S. M., & Wilkinson, I. (1980). Intraorganizational influence tactics: Explorations in getting one’s way. Journal of Applied Psychology, 65(4), 440-452. https://doi.org/10.1037/0021-9010.65.4.440

Kvale, S., & Brinkmann, S. (2009). InterViews: Learning the craft of qualitative research interviewing (2nd ed.). SAGE Publications.

Long, D., & Magerko, B. (2020). What is AI literacy? Competencies and design considerations. Proceedings of the 2020 CHI Conference on Human Factors in Computing Systems, 1-16. https://doi.org/10.1145/3313831.3376727

Malterud, K., Siersma, V. D., & Guassora, A. D. (2016). Sample size in qualitative interview studies: Guided by information power. Qualitative Health Research, 26(13), 1753-1760. https://doi.org/10.1177/1049732315617444

Naeem, M., Ozuem, W., Howell, K., & Ranfagni, S. (2023). A step-by-step process of thematic analysis to develop a conceptual model in qualitative research. International Journal of Qualitative Methods, 22, 16094069231205789. https://doi.org/10.1177/16094069231205789

Ng, D., Wu, W., Leung, J., Chiu, T., & Chu, S. K. (2023). Design and validation of the AI literacy questionnaire: The affective, behavioural, cognitive and ethical approach. British Journal of Educational Technology, 55(3), 1082-1104. https://doi.org/10.1111/bjet.13411

Pfeffer, J., & Salancik, G. R. (1978). The external control of organizations: A resource dependence perspective. Harper & Row.

Prensky, M. (2001). Digital natives, digital immigrants. On the Horizon, 9(5), 1-6. https://doi.org/10.1108/10748120110424816

Roberts, K., Dowell, A., & Nie, J. (2019). Attempting rigour and replicability in thematic analysis of qualitative research data: A case study of codebook development. BMC Medical Research Methodology, 19, 66. https://doi.org/10.1186/s12874-019-0707-y

Saunders, M., & Townsend, K. (2016). Reporting and justifying the number of interview participants in organization and workplace research. British Journal of Management, 27(4), 836-852. https://doi.org/10.1111/1467-8551.12182

Schroth, H. (2019). Are you ready for Gen Z in the workplace? California Management Review, 61(3), 5-18. https://doi.org/10.1177/0008125619841006

Scott, S. G., & Bruce, R. A. (1994). Determinants of innovative behavior: A path model of individual innovation in the workplace. Academy of Management Journal, 37(3), 580-607. https://doi.org/10.2307/256701

Seemiller, C., & Grace, M. (2016). Generation Z goes to college. Jossey-Bass.

Tripathi, N. (2021). Reverse the lens, set focus on the followers: A theoretical framework of resource dependence, upward influence, and leadership. Frontiers in Psychology, 12, 699340. https://doi.org/10.3389/fpsyg.2021.699340

Wang, B., Rau, P. L. P., & Yuan, T. (2022). Measuring user competence in using artificial intelligence: Validity and reliability of artificial intelligence literacy scale. Behaviour & Information Technology, 42(9), 1324-1337. https://doi.org/10.1080/0144929X.2022.2072768

Watkins, H., & Omilion-Hodges, L. (2025). Zooming in on generational differences: Exploring intergenerational employee relationships and the rise of Generation Z in the workplace. International Journal of Business Communication. https://doi.org/10.1177/23294884251345809

钟柏昌, 刘晓凡, 杨明欢. (2024). 何谓人工智能素养：本质、构成与评价体系. 华东师范大学学报（教育科学版）, (1), 71-84. https://doi.org/10.16382/j.cnki.1000-5560.2024.01.007

APPENDIXES

Appendix I: Interview Guide

Part A: Opening and Background (5 minutes)

Please briefly introduce yourself and your current work responsibilities.

How would you describe your experience with AI tools in your daily work?

Part B: AI Literacy (10 minutes)

How did you first learn to use AI tools like ChatGPT or others?

What AI tools do you use most frequently at work? For what purposes?

How would you rate your AI skills compared to your colleagues?

How would you rate your AI skills compared to your supervisor?

What is your general attitude toward AI technology? Any concerns or expectations?

Part C: Perceived Resource Advantage (10 minutes)

Do you feel that you have capabilities in AI that your supervisor lacks?

How did this perception develop? Can you describe specific situations?

How does this awareness affect how you see yourself in the organization?

Part D: Upward Influence Behavior (15 minutes)

Have there been times when your AI knowledge helped you influence your supervisor’s decisions or behaviors?

Can you describe a specific situation in detail? What did you do? What was the outcome?

What factors encouraged or discouraged you from attempting to influence your supervisor?

How do you typically approach making suggestions to your supervisor about AI-related matters?

Part E: Supervisor Openness (5 minutes)

How does your supervisor typically react to your AI-related suggestions?

Would you describe your supervisor as open to learning from younger employees? Why or why not?

How does your supervisor’s attitude affect your willingness to speak up?

Part F: Closing (5 minutes)

Is there anything else you would like to add about your experiences with AI at work?

Looking forward, how do you see your AI skills affecting your career development?

Appendix II: Informed Consent Form

研究参与知情同意书 / Informed Consent Form

研究题目 / Study Title: AI素养对Z世代员工向上影响行为的影响：资源依赖视角 The Impact of AI Literacy on Upward Influence Behavior among Generation Z Employees: A Resource Dependence Perspective

研究者 / Researcher: 

$$学生姓名$$

, 华侨学院

研究目的 / Purpose of the Study: 本研究旨在探索AI素养如何影响Z世代员工在工作中的向上影响行为。您的参与将帮助我们理解AI时代职场中代际动态的变化。

参与内容 / What Participation Involves: 您将参与一次约45-60分钟的深度访谈。访谈将以录音形式记录，之后转录为文字用于分析。

保密性 / Confidentiality: 所有收集的数据将严格保密。您的姓名和可识别信息将不会出现在任何研究报告中。录音文件将在转录完成后删除。

自愿参与 / Voluntary Participation: 您的参与完全自愿。您可以在任何时候选择退出研究，无需提供理由，这不会对您产生任何负面影响。

联系方式 / Contact Information: 如有任何问题，请联系研究者：

$$邮箱$$

同意声明 / Consent Statement: 我已阅读并理解上述信息。我同意自愿参与本研究，并同意访谈被录音。

签名 / Signature: ______ 日期 / Date: __

ACKNOWLEDGEMENTS

$$待完成$$

论文进度跟踪

已完成部分

| 章节 | 完成状态 | 备注 |
| 中文摘要 | ✅ 完成 | 可能需要根据最终发现调整 |
| 英文摘要 | ✅ 完成 | 可能需要根据最终发现调整 |
| Chapter I Introduction | ✅ 完成 | 研究背景、目的、意义 |
| Chapter II Literature Review | ✅ 完成 | 理论框架、文献综述 |
| Chapter III Methodology | ✅ 完成 | 研究设计、抽样、数据收集与分析 |
| Chapter IV Findings | ⏳ 待完成 | 需要完成访谈后填写 |
| Chapter V Discussion | ⏳ 待完成 | 需要完成数据分析后填写 |
| Chapter VI Conclusion | ⏳ 待完成 | 需要完成讨论后填写 |
| References | ✅ 完成 | 已更新补充文献 |
| Appendixes | ✅ 完成 | 访谈提纲、知情同意书 |
| Acknowledgements | ⏳ 待完成 | 最后填写 |

下一步行动

与导师确认研究设计：将开题报告和论文初稿给导师审阅

开始招募受访者：寻找符合条件的Z世代知识工作者

试访谈：先进行1-2次试访谈，测试访谈提纲

正式数据收集：完成15-20次深度访谈

数据分析：转录、编码、主题分析

撰写发现与讨论：完成Chapter IV-VI

关键时间节点

| 时间 | 任务 |
| 2024年12月 | 完成文献综述和访谈提纲定稿 |
| 2025年1月 | 试访谈并调整访谈问题 |
| 2025年2月 | 完成数据收集（15-20次访谈） |
| 2025年3月 | 完成数据分析和主题识别 |
| 2025年4月 | 完成论文初稿 |
| 2025年5月 | 根据导师反馈修改论文 |
| 2025年6月 | 提交最终论文并准备答辩 |

文档创建时间：2024年11月 最后更新：2024年11月（已补充理论细节）