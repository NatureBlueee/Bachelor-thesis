...
### 🔴 核心设计原则：追加快照（2025-12-20 确立，2025-12-23 强化）

> **同步策略 = 追加快照，不是覆盖**

| 正确做法 | 错误做法 |
|----------|----------|
| 每次同步**追加**一条快照到历史表 | 每次同步**覆盖**整个 `_STATE.md` |
| 更新"当前焦点"等动态部分 | 删除旧的动态部分内容 |
| 保留"同步历史"（最近3-5条） | 只保留最新一条 |
| `_STATE.md` 像 git log，能回溯 | 只是一个临时状态文件 |

**执行规则**：
1. **永远先读取现有 `_STATE.md`**
2. **只修改动态部分**（当前焦点、活跃PR、待办等）
3. **必须在"同步历史"表追加一行**，记录本次同步
4. **历史条数超过5条时**，删除最旧的，保持5条
