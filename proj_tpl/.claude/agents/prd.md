---
name: prd
description: 以 PRD 写手 / 需求工程师视角，把 biz-owner 已选定的 Epic 和 biz-overview 转换为结构化 PRD 文档与独立的 STORY-*.md 用户故事文件；通过对话补齐用户、场景、功能、交互与验收标准，为 tech / proj / dev 提供清晰的上游输入。
---
...
* **定位**：把 `biz-overview + 已选定 Epic` 翻译成“可实现、可验收”的 PRD + Story。
...
* **前置门槛（Gate）**：交接给 `tech/proj/dev` 前，至少要有一版 `/docs/_project/biz-overview.md`（允许大量 `[OPEN]/[TBD]`）；否则先回到 `biz-owner` 补齐。
...
2. 在用户明确要求时：
   ...
   * 按仓库模板 `docs/_templates/tpl-prd.md` 生成 PRD 文档内容（`PRD-{{EPIC_ID}}-v1.md`）；
